...
* [How to use](#how-to-use)
    * [Using an executable file from a released build](#using-an-executable-file-from-a-released-build)
...
None if using a released build (`.exe` file on Windows, binary executable on Mac or Linux).
...
There are two options: to use an executable (`.exe` file on Windows, binary executable on Mac or Linux) from a released build, or to run `python draw_io_parser.py` directly. Both are covered below. If you have any trouble running the parser, feel free to contact me directly at ricwil (at) arkivverket.no, or to use the RiC-O mailing list.

### Using an executable file from a released build

#### Preliminaries for Windows

Carry out the following steps.

* Download a release (likely the latest one) from github.
...
* Carry out the [preliminary steps](#how-to-use) described above, depending on whether you wish to use an executable file from a released build, or use Python directly.
* Pipe the contents of `example.drawio` into it in the terminal you opened in the preliminary steps above. Examples of how to do this are described below.

If using an executable file from a released build in Windows, proceed as follows (replacing `example.drawio` by a path to it if it is not in the same folder as `draw_io_parser.exe`).
...
The generated OWL outputted by the parser can for example be directed into a file, say with name `example.owl`. This would be as follows on Windows using an executable from a released build...
...
A number of command line arguments can be passed to the parser rather than only using in its most basic form as above.
...
Using an executable from a released build on Windows:
...
Run one of the following for an overview of all the possible command line arguments and an explanation of them. From an executable from a released build in Windows:
